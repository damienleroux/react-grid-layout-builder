...
function getReactGridLayoutFromProps(props) {
	const { children, updateConfigFunc, ...reactGridLayout } = props;
	return reactGridLayout;
}

const connectReactGridLayoutBuilder = ReactGridLayout => class extends Component {
	...
	updateLayoutIfNecessary = (layout, oldItem, newItem, placeholder, e, element) => {
		...
		const reactGridLayout = _.clone(getReactGridLayoutFromProps(this.props));
		const areSameItem = (itemA, itemB) => {
	...
	onDragStop = (layout, oldItem, newItem, placeholder, e, element) => {
		...
		const reactGridLayout = this.updateLayoutIfNecessary(layout, oldItem, newItem, placeholder, e, element);
	...
	render() {
		const { children, updateConfigFunc, ...reactGridLayout } = this.props;

		//avoid immutability issue to keep control on reactGridLayout future modifications
		const clonedReactGridLayout = _.cloneDeep(reactGridLayout);
		return (
			<ReactGridLayout
				{...clonedReactGridLayout}
